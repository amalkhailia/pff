 ...
 */
  exports.postCandidat = async (req,res) => {
   ...
   const responce = await newCandidat.save()
    res.status(200).send({msg:'candidat added successfully...',responce:responce})
...
if (!candidatToDelete)
{
    ...
    return
            ...
            res.status(400).send({msg:`can not find the candidat to delete ${error}`})
 ...
 */
 exports.UpdateOneCandidat = async (req,res) => {
    try { 
        const {id} = req.params
       const candidatToEdit = await Candidat.updateOne({_id:id},{$set:{...req.body}})
